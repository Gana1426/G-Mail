...
export async function GET(request: NextRequest) {
  return withMailAuth(request, async (_req, session) => {
    const result = await mailMessageService.listInbox(session, "ARCHIVE", {
      page: 1,
      limit: 50,
    });
...
export async function POST(request: NextRequest) {
  return withMailAuth(request, async (req, session) => {
    ...
    await mailMessageService.performAction(session, "archive", uids);
    return successResponse(null, "Messages archived");
  });
}
